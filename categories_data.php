<?php
/**
 * Fichier : categories_data.php
 * -----------------------------------
 * Contient 31 catégories, chacune avec ses sous-catégories.
 * À inclure partout (inscription, recherche, profil, etc.).
 */

$categoriesData = [

  /* --------------------------------------------------------------------
   * 1) Plomberie
   * ------------------------------------------------------------------*/
  "Plomberie" => [
    "Installation d'évier",
    "Installation de WC",
    "Dépannage de fuites",
    "Réparation de canalisations",
    "Rénovation de salle de bains",
    "Remplacement de robinets",
    "Débouchage de canalisations",
    "Inspection par caméra",
    "Remplacement de chauffe-eau",
    "Installation de ballon d'eau chaude",
    "Installation de compteurs d'eau",
    "Mise en place d'adoucisseur d'eau",
    "Remplacement de joints",
    "Pose de mitigeurs thermostatiques",
    "Entretien de fosses septiques"
  ],

  /* 2) Électricité */
  "Électricité" => [
    "Mise aux normes",
    "Dépannage électrique",
    "Installation luminaires",
    "Remplacement de tableaux",
    "Réparation de prises",
    "Branchement de VMC",
    "Installation de domotique",
    "Câblage réseau",
    "Pose d'alarmes",
    "Éclairage extérieur",
    "Remplacement de fusibles",
    "Diagnostic électrique",
    "Installation de planchers chauffants",
    "Chauffage électrique",
    "Entretien de générateurs"
  ],

  /* 3) Informatique */
  "Informatique" => [
    "Maintenance PC",
    "Développement web",
    "Réseaux et serveurs",
    "Sécurité informatique",
    "Installation système",
    "Réparation d'ordinateurs",
    "Sauvegarde et récupération de données",
    "Installation de logiciels",
    "Assistance à distance",
    "Virtualisation",
    "Configuration e-mails",
    "Support bureautique",
    "Installation parc informatique",
    "Audit de performance",
    "Optimisation OS"
  ],

  /* 4) Graphisme */
  "Graphisme" => [
    "Création de logos",
    "Design de flyers",
    "Branding",
    "Illustration",
    "Retouche photo",
    "Maquettes web",
    "Identité visuelle",
    "Conception d'affiches",
    "Charte graphique",
    "Packaging",
    "Infographie 2D",
    "Vectorisation",
    "Motion design (basique)",
    "Bannières réseaux sociaux",
    "Montage visuel"
  ],

  /* 5) Marketing */
  "Marketing" => [
    "SEO",
    "Réseaux Sociaux",
    "Publicité en ligne",
    "Email marketing",
    "Création de landing pages",
    "Automatisation marketing",
    "Copywriting",
    "Marketing d’influence",
    "Études de marché",
    "Growth Hacking",
    "Publicité Facebook/Instagram",
    "Référencement local",
    "Stratégie de contenu",
    "Gestion de newsletters",
    "Analyse statistique"
  ],

  /* 6) Menuiserie */
  "Menuiserie" => [
    "Pose de portes",
    "Fabrication de meubles",
    "Installation de fenêtres",
    "Agencement d'intérieur",
    "Rénovation de parquet",
    "Création d'escaliers",
    "Placards sur mesure",
    "Pose de volets",
    "Terrasses et bardage",
    "Fabrication de comptoirs",
    "Pose de plinthes",
    "Entretien bois extérieur",
    "Restaurations d'armoires",
    "Isolation phonique en bois",
    "Trappe de toit"
  ],

  /* 7) Peinture */
  "Peinture" => [
    "Peinture intérieure",
    "Peinture extérieure",
    "Effets décoratifs",
    "Rénovation de plafonds",
    "Pose de papier peint",
    "Restauration de boiseries",
    "Peinture antirouille",
    "Peinture hydrofuge",
    "Peinture de façade",
    "Enduits décoratifs",
    "Peinture sol",
    "Finitions laquées",
    "Vernis et lasure",
    "Peinture au pistolet",
    "Préparation murs (rebouchage)"
  ],

  /* 8) Maçonnerie */
  "Maçonnerie" => [
    "Construction de murs",
    "Création d’ouvertures",
    "Pose de briques",
    "Rénovation de fondations",
    "Dallage béton",
    "Murets de clôture",
    "Construction de piliers",
    "Agrandissement de maison",
    "Ravalement de façade",
    "Réparation de fissures",
    "Montage de cloisons",
    "Aménagement de combles",
    "Escaliers béton",
    "Chapes liquides",
    "Carottage et percement"
  ],

  /* 9) Jardinage */
  "Jardinage" => [
    "Tonte de pelouse",
    "Taille de haies",
    "Aménagement paysager",
    "Plantation d'arbustes",
    "Création de massifs",
    "Installation d'arrosage automatique",
    "Désherbage",
    "Entretien de potager",
    "Élagage d'arbres",
    "Scarification de pelouse",
    "Pose de gazon synthétique",
    "Bêchage",
    "Traitement maladies",
    "Allées et bordures",
    "Engazonnement"
  ],

  /* 10) Nettoyage */
  "Nettoyage" => [
    "Nettoyage industriel",
    "Nettoyage résidentiel",
    "Nettoyage voiture",
    "Nettoyage de vitres",
    "Désinfection",
    "Nettoyage après chantier",
    "Polissage de marbre",
    "Entretien de bureaux",
    "Traitement anti-acariens",
    "Débarras et évacuation",
    "Lavages haute pression",
    "Nettoyage de façades",
    "Assainissement de caves",
    "Nettoyage de parkings"
  ],

  /* 11) Serrurerie */
  "Serrurerie" => [
    "Ouverture de porte",
    "Remplacement de serrures",
    "Pose de verrous",
    "Serrures multipoints",
    "Blindage de portes",
    "Réparation après effraction",
    "Fermetures provisoires",
    "Cylindres haute sécurité",
    "Dépannage coffre-fort",
    "Serrures carénées",
    "Clés minutes",
    "Conseils en sécurité",
    "Maintenance digicodes",
    "Pose de gâches électriques",
    "Sécurisation de portes"
  ],

  /* 12) Carrelage */
  "Carrelage" => [
    "Pose de carrelage mural",
    "Carrelage de sol",
    "Faïence de cuisine",
    "Carreaux de ciment",
    "Carrelage extérieur",
    "Pose de mosaïque",
    "Ragréage sol",
    "Étanchéité douche italienne",
    "Réparation de joints",
    "Remplacement de carreaux cassés",
    "Pose de plinthes carrelées",
    "Faïence grand format",
    "Pose de frises décoratives",
    "Carrelage en diagonale",
    "Carrelage terrasse"
  ],

  /* 13) Vitrerie */
  "Vitrerie" => [
    "Remplacement de vitrages",
    "Pose de double vitrage",
    "Installation de fenêtres PVC",
    "Découpe de verre sur mesure",
    "Pose de miroirs",
    "Réparation de baies vitrées",
    "Vitrage antieffraction",
    "Film teinté",
    "Remplacement joints silicone",
    "Pose de vitres décoratives",
    "Dépannage bris de glace",
    "Isolation phonique",
    "Création verrières intérieures",
    "Installation pare-douche",
    "Entretien de fenêtres"
  ],

  /* 14) Toiture */
  "Toiture" => [
    "Réparation de tuiles",
    "Démoussage toiture",
    "Pose de gouttières",
    "Isolation sous toiture",
    "Rehausse de toit",
    "Étanchéité toit terrasse",
    "Installation de fenêtres de toit",
    "Remplacement de faîtage",
    "Couverture ardoise",
    "Traitement de charpente",
    "Rénovation couverture zinc",
    "Pose de bardage",
    "Installation pare-pluie",
    "Chéneaux sur mesure",
    "Ramonage de cheminée"
  ],

  /* 15) Transport */
  "Transport" => [
    "Déménagement",
    "Transport privé",
    "Transport de marchandises",
    "Livraison express",
    "Courses urgentes",
    "Transport frigorifique",
    "Logistique et stockage",
    "Groupage marchandises",
    "Manutention",
    "Installation de mobilier",
    "Transport de colis volumineux",
    "Location de camion",
    "Transport international",
    "Navette aéroport",
    "Messagerie rapide"
  ],

  /* 16) Événementiel */
  "Événementiel" => [
    "Organisation de mariages",
    "Planification de séminaires",
    "Décoration d’événements",
    "Animation DJ",
    "Sonorisation et lumières",
    "Location de salles",
    "Gestion traiteur",
    "Création de stands",
    "Hôtes et hôtesses",
    "Photobooth",
    "Événements enfants",
    "Animations à thème",
    "Promotion et billetterie",
    "Suivi budgétaire",
    "Communication événementielle"
  ],

  /* 17) Coiffure & Beauté */
  "Coiffure & Beauté" => [
    "Coiffure à domicile",
    "Coloration et mèches",
    "Manucure/Pédicure",
    "Maquillage",
    "Soins esthétiques",
    "Pose d'extensions capillaires",
    "Barbier",
    "Soin du visage",
    "Épilation",
    "Relooking",
    "Tatouage éphémère",
    "Réflexologie plantaire",
    "Stylisme ongulaire",
    "Modelage et relaxation",
    "Diagnostic capillaire"
  ],

  /* 18) Mécanique */
  "Mécanique" => [
    "Entretien de voitures",
    "Réparation de freins",
    "Changement de pneus",
    "Diagnostic moteur",
    "Vidange et filtres",
    "Réglage moteur",
    "Électricité auto",
    "Carrosserie légère",
    "Recharge clim auto",
    "Remplacement batterie",
    "Courroie de distribution",
    "Réparation échappement",
    "Révision complète",
    "Réglage parallélisme",
    "Contrôle technique préparatoire"
  ],

  /* 19) Rénovation */
  "Rénovation" => [
    "Rénovation complète d'appartement",
    "Isolation thermique",
    "Abattage de cloisons",
    "Réfection de sols",
    "Réagréage",
    "Rénovation de combles",
    "Décapage peinture",
    "Plâtrerie",
    "Pose de faux-plafonds",
    "Installation verrières",
    "Enduit décoratif",
    "Modification de plan électrique",
    "Refonte de plomberie",
    "Gestion de chantier global",
    "Finitions diverses"
  ],

  /* 20) Décoration */
  "Décoration" => [
    "Home staging",
    "Conseil en couleurs",
    "Décoration intérieure",
    "Décoration événementielle",
    "Décoration florale",
    "Peinture décorative",
    "Relooking de meubles",
    "Agencement feng shui",
    "Luminaire design",
    "Confection de rideaux",
    "Papier peint panoramique",
    "Décoration d'anniversaire",
    "Objets décoratifs sur mesure",
    "Création de têtes de lit",
    "Décoration de vitrine"
  ],

  /* 21) Photographie */
  "Photographie" => [
    "Shooting portrait",
    "Photographie mariage",
    "Photographie culinaire",
    "Reportage événementiel",
    "Retouche photo avancée",
    "Photographie immobilière",
    "Studio photo mobile",
    "Photographie de mode",
    "Photographie newborn",
    "Album photo personnalisé",
    "Séance couple/engagement",
    "Photographie produits",
    "Photo corporate",
    "Photographie lifestyle",
    "Cours de photographie"
  ],

  /* 22) Sécurité */
  "Sécurité" => [
    "Installation caméras",
    "Alarmes anti-intrusion",
    "Systèmes incendie",
    "Gardiennage",
    "Contrôle d'accès",
    "Surveillance de chantiers",
    "Télésurveillance",
    "Vidéoprotection urbaine",
    "Installation coffres-forts",
    "Maintenance systèmes sécurité",
    "Barrières levantes",
    "Détection de fumée",
    "Sécurité informatique",
    "Interphonie et visiophonie",
    "Rondes de sécurité"
  ],

  /* 23) Boulangerie & Pâtisserie */
  "Boulangerie & Pâtisserie" => [
    "Fabrication de pains spéciaux",
    "Viennoiseries artisanales",
    "Gâteaux de mariage",
    "Macarons",
    "Pièces montées",
    "Chocolaterie",
    "Pâtisseries fines",
    "Cupcakes décorés",
    "Desserts à thème",
    "Tartes salées",
    "Snacking boulanger",
    "Glaces maison",
    "Sculptures en chocolat",
    "Ateliers pâtisserie",
    "Livraison de commandes"
  ],

  /* 24) Traduction */
  "Traduction" => [
    "Traduction de documents",
    "Interprétation simultanée",
    "Traduction sites web",
    "Relecture et correction",
    "Traductions techniques",
    "Sous-titrage vidéo",
    "Localisation logiciels",
    "Traduction assermentée",
    "Transcription audio",
    "Interprétation consécutive",
    "Adaptation publicitaire",
    "Traduction littéraire",
    "Relecture SEO",
    "Traduction juridique",
    "Service multilingue"
  ],

  /* 25) Conseil juridique */
  "Conseil juridique" => [
    "Rédaction de contrats",
    "Conseil en droit du travail",
    "Médiation et arbitrage",
    "Rédaction CGU/CGV",
    "Formalités d'entreprise",
    "Conseil en propriété intellectuelle",
    "Conseil en baux commerciaux",
    "Accompagnement litiges",
    "Conseil fiscal",
    "Création de statuts",
    "Droit de la famille",
    "Établissement de procès-verbaux",
    "Procédures contentieuses",
    "Relecture d'accords",
    "Constitution de dossiers juridiques"
  ],

  /* 26) Formation */
  "Formation" => [
    "Cours de langues",
    "Formation bureautique",
    "Formation développement web",
    "Préparation examens/concours",
    "Coaching scolaire",
    "Formation management",
    "Formation PAO/DAO",
    "Formation logiciels pros",
    "Ateliers en groupe",
    "Formation e-learning",
    "Formation en sécurité",
    "Formation en finance",
    "Formation ressources humaines",
    "Formation marketing digital",
    "Formation premiers secours"
  ],

  /* 27) Consulting business */
  "Consulting business" => [
    "Conseil en stratégie",
    "Audit financier",
    "Accompagnement startups",
    "Business plan",
    "Optimisation de processus",
    "Organisation et management",
    "Acquisition clients",
    "Études de faisabilité",
    "Coaching dirigeant",
    "Conseil en fusion/acquisition",
    "Analyse concurrentielle",
    "Gestion de projets",
    "Gestion du changement",
    "Relance d'activité",
    "Étude de rentabilité"
  ],

  /* 28) Musique et Spectacle */
  "Musique et Spectacle" => [
    "Cours de guitare",
    "Cours de piano",
    "Animation musicale événement",
    "Groupes de musique live",
    "Spectacle pour enfants",
    "Cours de chant",
    "Composition musicale",
    "Prestation DJ",
    "Karaoké pro",
    "Sonorisation concert",
    "Enregistrement studio",
    "Troupe de danse",
    "Performance de rue",
    "Magicien mentaliste",
    "Marionnettes & théâtre"
  ],

  /* 29) Sport et Coaching */
  "Sport et Coaching" => [
    "Coaching sportif individuel",
    "Préparation physique",
    "Perte de poids",
    "Entraîneur running",
    "Personal trainer à domicile",
    "Cours de yoga",
    "Cours de Pilates",
    "Coaching nutrition",
    "Rééducation sportive",
    "Cours de natation",
    "Fitness en groupe",
    "Cross-training",
    "Conseil en diététique",
    "Entraîneur e-sport",
    "Préparation compétitions"
  ],

  /* 30) Restauration et Traiteur */
  "Restauration et Traiteur" => [
    "Menus traiteur pour événements",
    "Cocktails dînatoires",
    "Chef à domicile",
    "Cuisine du monde",
    "Buffets froids et chauds",
    "Plateaux-repas entreprise",
    "Food truck",
    "Gastronomie haut de gamme",
    "Desserts personnalisés",
    "Animations culinaires",
    "Service de personnel en salle",
    "Cuisine végétarienne",
    "Brunchs et petits-déjeuners",
    "Service barbecue",
    "Menus adaptés régimes spéciaux"
  ],

  /* ----------------------------------------------------------
   * 31) Vente véhicule  (NOUVELLE CATÉGORIE)
   * --------------------------------------------------------*/
  "Vente véhicule" => [
    "Citadine",
    "Berline",
    "SUV / 4x4",
    "Utilitaire",
    "Moto / Scooter",
    "Véhicule électrique",
    "Véhicule hybride",
    "Voiture de collection",
    "Camping-car",
    "Pick-up",
    "Véhicule sans permis",
    "Autre type de véhicule",
    "Pièces détachées",
    "Accessoires auto/moto",
    "Services de reprise"
  ]

];

return $categoriesData;