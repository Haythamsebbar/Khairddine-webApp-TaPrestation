                            ...
                            <button class="flex items-center px-3 py-2 text-sm leading-4 font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200">
                                ...
                                <div class="flex-shrink-0 mr-2">
                                    @if(Auth::user()->hasRole('client') && Auth::user()->client && Auth::user()->client->avatar)
                                        <img class="h-8 w-8 rounded-full object-cover border-2 border-gray-200" src="{{ Storage::url(Auth::user()->client->avatar) }}" alt="{{ Auth::user()->name }}">
                                    @elseif(Auth::user()->profile_photo_path)
                                        <img class="h-8 w-8 rounded-full object-cover border-2 border-gray-200" src="{{ Storage::url(Auth::user()->profile_photo_path) }}" alt="{{ Auth::user()->name }}">
                        ...
                        <x-slot name="content">
                            ...
                            <div class="px-4 py-3 border-b border-gray-100">
                                <div class="flex items-center">
                                    @if(Auth::user()->hasRole('client') && Auth::user()->client && Auth::user()->client->avatar)
                                        <img class="h-10 w-10 rounded-full object-cover" src="{{ Storage::url(Auth::user()->client->avatar) }}" alt="{{ Auth::user()->name }}">
                                    @elseif(Auth::user()->profile_photo_path)
            ...
            <div class="pt-4 pb-1 border-t border-gray-200">
                ...
                <div class="space-y-1 px-4 pb-4">
                    ...
                    @elseif(Auth::user()->hasRole('client'))
                        ...
                        <a href="{{ route('client.bookings.index') }}" class="flex items-center px-4 py-3 text-gray-700 hover:text-blue-600 border-l-3 border-transparent hover:border-blue-600 transition-all duration-200">
                            <i class="fas fa-calendar-alt mr-3 text-gray-500 text-sm"></i>
                            Mes réservations
                        </a>
                        <a href="{{ route('client.equipment-rental-requests.index') }}" class="flex items-center px-4 py-3 text-gray-700 hover:text-blue-600 border-l-3 border-transparent hover:border-blue-600 transition-all duration-200">
                            <i class="fas fa-tools mr-3 text-gray-500 text-sm"></i>
                            Mes locations
                        </a>
                    @endif
