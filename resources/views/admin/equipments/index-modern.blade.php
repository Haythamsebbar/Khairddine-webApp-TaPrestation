        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                <div class="flex flex-wrap gap-2 sm:gap-3">
                    <a href="{{ route('administrateur.equipments.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center justify-center text-sm sm:text-base">
                    ...
                    <button class="bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center justify-center text-sm sm:text-base" onclick="exportEquipments()">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <div class="bg-white rounded-xl shadow-lg border border-blue-200 p-4 sm:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="text-xs sm:text-sm font-medium text-blue-600 uppercase tracking-wide">Total Équipements</div>
                        <div class="text-2xl sm:text-3xl font-bold text-blue-900 mt-1">{{ $equipments->total() ?? 0 }}</div>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-blue-200 p-4 sm:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-2xl sm:text-3xl font-bold text-blue-900 mt-1">{{ $equipments->where('status', 'available')->count() ?? 0 }}</div>
                        <div class="flex items-center mt-2 text-xs sm:text-sm text-green-600">
                            <i class="fas fa-arrow-up mr-1"></i>
                            <span>+8% ce mois</span>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-blue-200 p-4 sm:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-2xl sm:text-3xl font-bold text-blue-900 mt-1">{{ $equipments->where('status', 'maintenance')->count() ?? 0 }}</div>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-blue-200 p-4 sm:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="text-xs sm:text-sm font-medium text-blue-600 uppercase tracking-wide">Valeur Totale</div>
        ...
        <div id="filtersPanel" class="bg-white rounded-xl shadow-lg border border-blue-200 p-4 sm:p-6 mb-6 sm:mb-8" style="display: none;">
            <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="text-center sm:text-left">
                    ...
                    <p class="text-sm sm:text-base lg:text-lg text-blue-700">Affinez votre recherche pour trouver l'équipement parfait</p>
                </div>
            ...
            <form action="{{ route('administrateur.equipments.index') }}" method="GET" class="space-y-4 sm:space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="owner" id="owner" value="{{ request('owner') }}" placeholder="Nom du propriétaire..." class="w-full pl-10 pr-4 py-2.5 sm:py-3 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 text-sm sm:text-base">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 sm:pt-6 border-t-2 border-blue-200">
                    ...
                    @if(request('name') || request('type') || request('status') || request('owner') || request('price_min') || request('price_max'))
                        <a href="{{ route('administrateur.equipments.index') }}" class="bg-white hover:bg-gray-50 text-blue-600 border border-blue-200 font-bold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg transition duration-200 flex items-center justify-center text-sm sm:text-base">
...
<div class="bg-white rounded-xl shadow-lg border border-blue-200 p-6">
    <div class="flex justify-between items-center mb-6">
        <div class="text-xl font-bold text-blue-800">Liste des équipements ({{ $equipments->total() ?? 0 }})</div>
        <div class="flex gap-4 items-center">
            ...
            <button class="bg-blue-100 hover:bg-blue-200 text-blue-800 font-bold py-2 px-4 rounded-lg transition duration-200 flex items-center" onclick="exportEquipments()">
                <i class="fas fa-download mr-2"></i>
                Exporter
            </button>
        </div>
    ...
    <div class="space-y-4">
            ...
            <div class="bg-white border border-blue-200 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 p-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4 flex-1">
                        <input type="checkbox" name="selected_equipments[]" value="{{ $equipment->id }}" class="equipment-checkbox w-4 h-4 text-blue-600 border-blue-300 rounded focus:ring-blue-500">
                        
                        <!-- Vignette de l'équipement -->
                        <div class="flex-shrink-0">
                            @if($equipment->image)
                                <img src="{{ asset('storage/' . $equipment->image) }}" alt="{{ $equipment->name }}" class="w-16 h-16 rounded-lg object-cover border border-blue-200">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between mb-2">
                                <div class="flex-1">
                                    <h3 class="text-lg font-bold text-blue-900 truncate">{{ $equipment->name }}</h3>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                                            {{ ucfirst($equipment->type ?? 'Non défini') }}
                                        </span>
                            ...
                            <p class="text-sm text-gray-600 line-clamp-2 mb-3">{{ Str::limit($equipment->description, 120) }}</p>
                            
                            <!-- Métriques -->
                            <div class="flex items-center gap-4 text-xs text-gray-500">
                                <div class="flex items-center gap-1">
                                    <i class="fas fa-euro-sign text-blue-500"></i>
                                    <span>{{ $equipment->daily_price ?? 0 }}€/jour</span>
                                </div>
                                <div class="flex items-center gap-1">
                                    <i class="fas fa-calendar-check text-blue-500"></i>
                                    <span>{{ $equipment->rentals_count ?? 0 }} locations</span>
                                </div>
                                <div class="flex items-center gap-1">
                                    <i class="fas fa-user text-blue-500"></i>
                                    <span>{{ $equipment->owner->name ?? 'N/A' }}</span>
                                </div>
                                <div class="flex items-center gap-1">
                                    <i class="fas fa-calendar text-blue-500"></i>
                                    <span>Ajouté {{ $equipment->created_at->diffForHumans() }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 ml-4">
                        <a href="{{ route('administrateur.equipments.show', $equipment->id) }}" class="bg-blue-100 hover:bg-blue-200 text-blue-800 p-2 rounded-lg transition duration-200" title="Voir les détails">
                            <i class="fas fa-eye"></i>
                        </a>
                        <a href="{{ route('administrateur.equipments.edit', $equipment->id) }}" class="bg-yellow-100 hover:bg-yellow-200 text-yellow-800 p-2 rounded-lg transition duration-200" title="Modifier">
                            <i class="fas fa-edit"></i>
                        </a>
                        <button onclick="deleteEquipment({{ $equipment->id }})" class="bg-red-100 hover:bg-red-200 text-red-800 p-2 rounded-lg transition duration-200" title="Supprimer">
        ...
        @empty
            <div class="text-center py-12">
                ...
                <a href="{{ route('administrateur.equipments.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                    <i class="fas fa-plus mr-2"></i>
    ...
    <!-- Pagination -->
    @if(isset($equipments) && $equipments->hasPages())
