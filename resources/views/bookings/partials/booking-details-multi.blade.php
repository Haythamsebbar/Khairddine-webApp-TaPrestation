...
<div class="bg-white rounded-lg shadow-sm border p-4 w-full max-w-md mx-auto">
    ...
    <div class="space-y-3">
        ...
        <div class="flex items-center justify-between py-2 border-b border-gray-100">
            <div class="flex items-center">
                <i class="fas fa-list text-blue-500 mr-2 w-4 text-sm"></i>
                <span class="text-sm text-gray-600">Créneaux</span>
            </div>
            <span class="text-sm font-medium text-gray-900">{{ $allBookings->count() }} créneaux</span>
        </div>
        
        <!-- Période totale -->
        <div class="flex items-center justify-between py-2 border-b border-gray-100">
            <div class="flex items-center">
                <i class="fas fa-calendar-week text-green-500 mr-2 w-4 text-sm"></i>
                <span class="text-sm text-gray-600">Période</span>
            </div>
            <div class="text-right">
                <div class="text-sm font-medium text-gray-900">{{ $allBookings->first()->start_datetime->format('d/m/Y') }}</div>
                @if($allBookings->first()->start_datetime->format('Y-m-d') !== $allBookings->last()->start_datetime->format('Y-m-d'))
                    <div class="text-xs text-gray-500">au {{ $allBookings->last()->start_datetime->format('d/m/Y') }}</div>
                @endif
            </div>
        </div>
        ...
        <div class="flex items-center justify-between py-2 border-b border-gray-100">
            ...
            <span class="text-sm font-bold text-green-600">{{ number_format($totalSessionPrice, 2) }} €</span>
        </div>
        ...
        <div class="flex items-center justify-between py-2">
            ...
            <span class="text-sm font-medium text-gray-900">{{ $currentBooking->created_at->format('d/m/Y') }}</span>
        </div>
    </div>
    ...
    <div class="mt-4 pt-4 border-t border-gray-100 text-center">
        ...
        <div class="bg-blue-50 rounded-lg p-3 text-center">
            <h4 class="font-medium text-blue-900 text-sm">{{ $currentBooking->service->name }}</h4>
            @if($currentBooking->service->description)
                <p class="text-xs text-blue-700 mt-1 line-clamp-2">{{ Str::limit($currentBooking->service->description, 100) }}</p>
            @endif
