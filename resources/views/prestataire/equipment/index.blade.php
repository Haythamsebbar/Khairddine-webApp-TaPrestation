...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-green-600">Mes équipements</h1>
        <a href="{{ route('prestataire.equipment.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition duration-200">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600">Total des équipements</p>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['rented'] ?? 0 }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form method="GET" action="{{ route('prestataire.equipment.index') }}" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-64">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher par nom..." class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
            </div>
            
            <div>
                <select name="category" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
            ...
            <div>
                <select name="sort" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
            ...
            @if(request()->hasAny(['search', 'status', 'category', 'sort']))
                <a href="{{ route('prestataire.equipment.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-4 py-2 rounded-md transition duration-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($equipment as $item)
                <div class="bg-white rounded-lg shadow hover:shadow-lg transition duration-200 flex flex-col">
                    ...
                    <div class="relative">
                        @if($item->main_photo)
                            <img src="{{ Storage::url($item->main_photo) }}" alt="{{ $item->name }}" class="w-full h-48 object-cover rounded-t-lg">
                        @else
                    ...
                    <div class="p-4 flex-grow">
                        <h3 class="font-semibold text-lg text-gray-900 mb-2 line-clamp-2">{{ $item->name }}</h3>
                    ...
                    <div class="p-4 bg-gray-50 rounded-b-lg border-t border-gray-200">
                        <div class="flex gap-2">
                            <a href="{{ route('prestataire.equipment.show', $item) }}" class="flex-1 bg-green-600 hover:bg-green-700 text-white text-center py-2 rounded-md transition duration-200 text-sm">
                                <i class="fas fa-eye mr-1"></i>Voir
                            </a>
                            <a href="{{ route('prestataire.equipment.edit', $item) }}" class="flex-1 bg-gray-600 hover:bg-gray-700 text-white text-center py-2 rounded-md transition duration-200 text-sm">
    ...
    @else
        <div class="text-center py-12">
            ...
            <h3 class="text-xl font-semibold text-gray-900 mb-2">Aucun équipement trouvé</h3>
            <p class="text-gray-600 mb-6">Vous n'avez pas encore ajouté d'équipement.</p>
            <a href="{{ route('prestataire.equipment.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg transition duration-200">
                <i class="fas fa-plus mr-2"></i>Ajouter un équipement
            </a>
        </div>
    @endif
